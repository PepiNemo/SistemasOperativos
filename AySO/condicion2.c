...
void *incrementa_count(void *tid) {
  ...
  for (i=0; i < TCOUNT; i++) {
    ...
    contador++;
    //Revisa el valor de contador y hace signal al thread en espera cuando se satisface la condicion. Esto ocurre mientras mutex esta bloqueado.
    if (contador == COUNT_LIMIT){
      printf("incrementa_count(): thread %ld, contador = %d  Umbral alcanzado. ", my_id, contador);
